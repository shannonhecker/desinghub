...
import {
  UOAUI_THEMES, uoauiBuildCSS, UOAUI_COMPS, UOAUI_CATS, AIcon, UOAUI_FONT,
  setUoauiT, getUoauiT, getUoauiPreviews, getUoauiDemoComponent,
  getUoauiDensityCSS,
  UOAUI_MOTION, UOAUI_OPACITY, UOAUI_RADIUS, UOAUI_SHADOW, UOAUI_BORDER,
} from './uoaui/uoaui-documentation.jsx';
...
export {
  ...
  CARBON_MOTION, CARBON_TYPE, CARBON_RADIUS, CARBON_SPACING, CARBON_BORDER,
  SALT_MOTION, SALT_CURVE, SALT_ELEVATION, SALT_TYPE, SALT_BORDER,
  getSaltPreviews, getFluentPreviews, getUoauiPreviews, getCarbonPreviews,
};
...
export function getComponents(system: SystemId): ComponentDef[] {
  ...
  const compsBySystem: Record<SystemId, RawComponent[]> = {
    salt: SALT_COMPS, m3: M3_COMPS, fluent: FLUENT_COMPS, uoaui: UOAUI_COMPS, carbon: CARBON_COMPS,
  };
...
export function getThemeKeys(system: SystemId): string[] {
  const dictBySystem: Record<SystemId, ThemeDict> = {
    salt: SALT_THEMES, m3: M3_THEMES, fluent: FLUENT_THEMES, uoaui: UOAUI_THEMES, carbon: CARBON_THEMES,
  };
...
export function getTheme(system: SystemId, themeKey: string, customColor?: string, isDarkCustom?: boolean): ThemeTokens {
  const st = SALT_THEMES as ThemeDict, mt = M3_THEMES as ThemeDict, ft = FLUENT_THEMES as ThemeDict, at = UOAUI_THEMES as ThemeDict, ct = CARBON_THEMES as ThemeDict;
  switch (system) {
    ...
    case 'uoaui': return at[themeKey] || at['dark'];
...
export function activateTheme(system: SystemId, theme: ThemeTokens) {
  switch (system) {
    ...
    case 'uoaui': setUoauiT(theme); break;
...
export function getDemoComponent(system: SystemId, componentId: string): React.ComponentType | null {
  switch (system) {
    ...
    case 'uoaui': return getUoauiDemoComponent(componentId);
...
export function getPreviews(system: SystemId): Record<string, React.ComponentType> {
  switch (system) {
    ...
    case 'uoaui': return getUoauiPreviews();
...
export function getFullCSS(system: SystemId, theme: ThemeTokens, densityOrSize: string | number): string {
  switch (system) {
    ...
    case 'uoaui': return uoauiBuildCSS(theme) + getUoauiDensityCSS(densityOrSize as string);
...
export function getSystemInfo(system: SystemId) {
  const info = {
    ...
    uoaui: { name: "uoaui DS", org: "uoaui", color: "#8A58C9", icon: "A" },
...
const UOAUI_DENSITY_MAP: Record<string, DensityEntry> = {
...
export { SALT_DENSITY_MAP, UOAUI_DENSITY_MAP, CARBON_DENSITY_MAP };
export { SALT_THEMES, M3_THEMES, FLUENT_THEMES, UOAUI_THEMES, CARBON_THEMES };
